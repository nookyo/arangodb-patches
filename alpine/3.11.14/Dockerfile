################################################################
  ...
  # 2) Копируем собранный артефакт
  COPY --from=builder /opt/arangodb-${TAG} /opt/arangodb-${TAG}

  # 3) Делаем бинарники ArangoDB доступными в PATH
  RUN for i in $(ls /opt/arangodb-${TAG}/bin); do \
        ln -s /opt/arangodb-${TAG}/bin/$i /usr/bin/$i; \
      done && \
      ln -s /opt/arangodb-${TAG}/sbin/arangod /usr/bin/arangod
  ...
  COPY alpine/3.11.14/docker-entrypoint.sh /entrypoint.sh
  COPY alpine/3.11.14/docker-foxx.sh /usr/bin/foxx
